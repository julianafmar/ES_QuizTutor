...
Cypress.Commands.add('addStudentStats', (academicTerm, numStudents, numMore75CorrectQuestions, numAtLeast3Quizzes) => {
  dbCommand(`UPDATE student_stats
              SET num_at_least3quizzes = ${numAtLeast3Quizzes}, 
                  num_more75correct_questions = ${numMore75CorrectQuestions}, 
                  num_students = ${numStudents}
              WHERE course_execution_id = (SELECT id FROM course_executions WHERE academic_term = '${academicTerm}');`);
});


Cypress.Commands.add('addQuizStats', (academicTerm, numQuizzes, numUniqueAnsweredQuizzes, averageQuizzesSolved) => {
  dbCommand(`UPDATE quiz_stats
              SET average_quizzes_solved = ${averageQuizzesSolved}, 
                  num_unique_answered_quizzes = ${numUniqueAnsweredQuizzes}, 
                  num_quizzes = ${numQuizzes}
              WHERE course_execution_id = (SELECT id FROM course_executions WHERE academic_term = '${academicTerm}');`);
});


Cypress.Commands.add('addQuestionStats', (academicTerm, numAvailable, answeredQuestionsUnique, averageQuestionsAnswered) => {
  dbCommand(`UPDATE question_stats
              SET answered_questions_unique = ${answeredQuestionsUnique}, 
                  average_questions_answered = ${averageQuestionsAnswered}, 
                  num_available = ${numAvailable}
              WHERE execution_id = (SELECT id FROM course_executions WHERE academic_term = '${academicTerm}');`);
});


Cypress.Commands.add('createTeacherDashboard', (academicTerm) => {
  dbCommand(`INSERT INTO teacher_dashboard (id, course_execution_id, teacher_id)
              SELECT teacher_dashboard.id+1, course_execution_id, 1 
              FROM course_executions 
              JOIN teacher_dashboard ON course_executions.id = teacher_dashboard.course_execution_id
              WHERE course_executions.academic_term = '${academicTerm}'
              AND teacher_dashboard.id = (SELECT max(id) FROM teacher_dashboard);`);
});


Cypress.Commands.add('createDemoCourse2', () => {
  cy.task('queryDatabase', {
    query: `INSERT INTO courses
            SELECT id+1, 'Demo Course 2', 'TECNICO' FROM courses
...
Cypress.Commands.add('createCourseExecutionOnDemoCourse2', (academicTerm, endDate) => {
  cy.task('queryDatabase', {
    query: `INSERT INTO course_executions (id, academic_term, acronym, end_date, status, type, course_id)
            SELECT course_executions.id+1, '${academicTerm}', 'DemoCourse2', TO_TIMESTAMP('${endDate}', 'YYYY-MM-DD HH:MI:SS'), 'ACTIVE', 'TECNICO', courses.id 
            FROM course_executions
            JOIN courses ON courses.name = 'Demo Course 2'
              WHERE course_executions.id=(SELECT max(id) FROM course_executions)
              AND courses.id=(SELECT id from courses WHERE name = 'Demo Course 2')`,
    credentials: credentials,
  });
});
...
Cypress.Commands.add('deleteCourseExecutions', () => {
  dbCommand(`DELETE FROM teacher_dashboard_student_stats;
              ...
              DELETE FROM teacher_dashboard
                WHERE course_execution_id IN (SELECT id FROM course_executions
                                                WHERE academic_term != '1st Semester');
              ...
              DELETE FROM course_executions 
                WHERE academic_term != '1st Semester';
              DELETE FROM courses
                WHERE name = 'Demo Course 2';
            `);
});


Cypress.Commands.add('removeDemoCourseExecution', () => {
  dbCommand(`DELETE FROM users_course_executions
              WHERE course_executions_id = 
                (SELECT id FROM course_executions WHERE academic_term = '1st Semester');`);
});

Cypress.Commands.add('addDemoCourseExecution', () => {
  dbCommand(`INSERT INTO users_course_executions
              SELECT users.id, course_executions.id FROM course_executions
              JOIN users ON users.name = 'Demo Teacher'
              WHERE course_executions.id = (SELECT id FROM course_executions WHERE academic_term = '1st Semester');`);
});
  ...
  (academicTerm) => {
    cy.task('queryDatabase', {
      query: `UPDATE users_course_executions
            SET course_executions_id=(SELECT id FROM course_executions 
...
Cypress.Commands.add('addDemoTeacherCourseExecution', (academicTerm) => {
  cy.task('queryDatabase', {
    query: `INSERT INTO users_course_executions (users_id, course_executions_id) 
              SELECT users.id, course_executions.id 
              FROM users 
              JOIN course_executions ON course_executions.academic_term = '${academicTerm}' 
              WHERE users.name='Demo Teacher';`,
    credentials: credentials,
  });
});